...
rem   jar    - java -jar thin target\quarkus-app\quarkus-run.jar, fallback uber-jar target\javatool-mcp-1.0.0-SNAPSHOT-runner.jar (default)
rem   binary - target\javatool-mcp.exe (GraalVM native image)
rem Related env: CONSOLE_PORT, CONSOLE_TOKEN, DATA_DIR,
rem              JAVA_HOME (JDK 21), MAVEN_HOME (defaults to D:\Soft\Maven3)
rem ============================================================
setlocal
cd /d "%~dp0"
...
if /i "%BACKEND_LAUNCH_MODE%"=="binary" goto binary
...
rem 优先瘦 jar（quarkus-app 目录），不存在则回退 uber-jar（target\javatool-mcp-1.0.0-SNAPSHOT-runner.jar）
set "RUN_JAR=target\quarkus-app\quarkus-run.jar"
if exist "%RUN_JAR%" goto jar_run
set "RUN_JAR=target\javatool-mcp-1.0.0-SNAPSHOT-runner.jar"
if exist "%RUN_JAR%" goto jar_run
echo [start.bat] Backend jar not found: target\quarkus-app\quarkus-run.jar (thin) or target\javatool-mcp-1.0.0-SNAPSHOT-runner.jar (uber)
...
:binary
if not exist "target\javatool-mcp.exe" (
    echo [start.bat] Native binary not found: target\javatool-mcp.exe
    echo [start.bat] Run "mvn package -Dnative" first, or set BACKEND_LAUNCH_MODE=jar
    exit /b 1
)
echo [start.bat] Launching backend in BINARY mode...
target\javatool-mcp.exe
goto :eof